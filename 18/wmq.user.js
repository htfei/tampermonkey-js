...
// @match        https://p1.jlgqrvxf.xyz/*
// @include      /^http(s)?:\/\/p4\.\w+\.(com|cc|xyz)/
...
(function() {
    ...
    function show_tips(m3u8url){
        let nodes = document.querySelectorAll("div.swiper-slide-active");
        for(var i = 0 ; i < nodes.length ; i++){
            let cur = nodes[i];
            window.now_node= cur;
            //console.log("now node:",cur);
            cur.querySelector("div.index_item_bottom")?.remove();//去除热点推荐
            let vip = cur.querySelector("div.index_info_bottom_vip_tips span"); if(vip) vip.innerText = `✅已破解`;//破解提示
        }
        // 获取 video 元素
        const video = document.querySelector('video')
        // 添加自定义属性
        if (video) {
            video.dataset.filename = document.querySelector("div.swiper-slide.swiper-slide-active > div > div.index_item_info > p")?.innerText || document.title;;
