...
(function () {
    ...
    function get_videourl() {
        ...
        try {
            ...
            if (location.href.match("https://.*?/index/movie/play/id/")) {
                ...
                /* 3.vip视频需要移除登录框 */
                ads = document.querySelector("#login-tip-modal"); ads && ads.parentNode.removeChild(ads);
                ads = document.querySelector(".vip-tip-modal"); ads && ads.parentNode.removeChild(ads);
                ads = document.querySelector(".pause-ad-imgbox"); ads && ads.parentNode.removeChild(ads);
                /* 4.播放正片 */
            ...
            /* 快猫，兼容手机 + PC */
                ...
                /* 3.移除广告 */
                ads = document.querySelector(".exchangeBg"); ads.style.display = "none";
                /* 4.播放正片 */
            ...
            /* 红杏，兼容手机 + PC */
                ...
                /* 4.播放正片 */
                player = document.querySelector(" div#mse") || document.querySelector("div.play_video_1 div.dplayer");
                dizhi = document.querySelector("div.move_name") || document.querySelector("div.play_main_1");
            ...
            /* 含羞草视频 ，兼容手机 + PC */
                ...
                ads = document.querySelector(".fixedTryWatchShowButtonLine"); ads && (ads.style.display = "none");
                /* 4.播放正片 */
    ...
    function play_video(videoUrl, el, dizhi) {
        ...
        if (!videoUrl || !el || !dizhi) throw new Error(`某些参数无效，视频地址：${videoUrl}、播放器加载位置：${el}、提示位置：${dizhi}`);
        ...
        mydiv.innerHTML = `<div id="my_add_dizhi" style="color:red;font-size:14px">
            ...
            <p style="color:red;font-size:14px">提示：各位彦祖，部分网站需要登录后使用，出现卡顿、不断刷新等情况刷新页面即可解决！问题反馈 or 支持作者请
    ...
    function show_err_log(err) {
        ...
        mydiv.innerHTML = `<div id="my_add_err_log" style="color:red;font-size:14px">
            脚本在解析视频地址的过程中发生错误，请复制以下内容到脚本评论区联系作者解决!
            <p style="color:red;font-size:14px">错误打印：${err}</p>
            <p style="color:red;font-size:14px">操作系统：${err}</p>
            <p style="color:red;font-size:14px">浏览器：${err}</p>
            </div>`;
