...
var ajaxHooker = function() {
        ...
        send(data) {
            ...
            for (var key in request.headers) {
                if (request.headers.hasOwnProperty(key)) {
                    originalHeaders[key] = request.headers[key];
                }
            }
            ...
            try {
                for (var inst of hookInsts) {
                    const ahReq = new AHRequest(request);
                    if (ahReq.shouldFilter(inst.filters)) continue;
                    for (var i = 0; i < inst.hookFns.length; i++) {
                        const fn = inst.hookFns[i];
                        ...
                        if (fnType === '[object Function]' || fnType === '[object AsyncFunction]') {
                            try {
                                ...
                                console.error('hook function error:', e);
                            }
                        }
                    }
                }
            } catch (e) {
                console.error('sync hook execution error:', e);
            }
            
            // 检查请求是否被修改
            const urlChanged = request.url !== originalUrl;
            const methodChanged = request.method !== originalMethod;
            const dataChanged = request.data !== originalData;
            const headersChanged = Object.keys(request.headers).length !== Object.keys(originalHeaders).length ||
                Object.keys(request.headers).some(function(key) {
                    return request.headers[key] !== originalHeaders[key];
                });
            const requestModified = urlChanged || methodChanged || dataChanged || headersChanged;
    ...
    function fakeXHR() {
        ...
        xhr.__ajaxHooker_fromFakeXHR = true; // 标记来自fakeXHR，避免原型链hook重复处理
    ...
    // 额外：修改原型链，确保缓存请求也能被捕获（只安装一次）
    if (!winAh.protoHookInstalled) {
        winAh.protoHookInstalled = true;
        
        const originalProtoOpen = winAh.realXHR.prototype.open;
        const originalProtoSend = winAh.realXHR.prototype.send;
        winAh.realProtoOpen = originalProtoOpen;
        winAh.realProtoSend = originalProtoSend;
        
        // 保存原始的responseText getter
        const originalResponseTextDescriptor = Object.getOwnPropertyDescriptor(winAh.realXHR.prototype, 'responseText');
        winAh.realResponseTextGetter = originalResponseTextDescriptor ? originalResponseTextDescriptor.get : null;
    
    // 重写原型open方法
        winAh.realXHR.prototype.open = function(method, url, async, user, password) {
            this.__ajaxHooker_url = url;
            this.__ajaxHooker_method = method;
            this.__ajaxHooker_request = null;
            
            // 预先初始化responseText相关变量
            this.__ajaxHooker_originalResponseText = undefined;
            this.__ajaxHooker_modifiedResponseText = undefined;
            
            return originalProtoOpen.apply(this, arguments);
        };
    
    // 重写responseText getter
    if (winAh.realResponseTextGetter) {
        Object.defineProperty(winAh.realXHR.prototype, 'responseText', {
            get: function() {
                // 先获取原始值
                const originalValue = winAh.realResponseTextGetter.call(this);
                
                // 如果是m3u8请求，且还没有修改，且readyState=4，尝试同步修改
                if (this.__ajaxHooker_url && this.__ajaxHooker_url.indexOf('.m3u8') !== -1 && 
                    this.__ajaxHooker_modifiedResponseText === undefined && 
                    this.readyState === 4 &&
                    !this.__ajaxHooker_responseProcessed) {
                    
                    // 标记已处理，避免重复调用
                    this.__ajaxHooker_responseProcessed = true;
                    
                    // 触发response回调
                    const request = this.__ajaxHooker_request;
                    if (request && typeof request.response === 'function') {
                        ...
                        const result = request.response(response);
                        if (typeof result === 'string') {
                            this.__ajaxHooker_modifiedResponseText = result;
                        }
                    }
                }
                
                // 如果有修改后的内容，返回修改后的
                if (this.__ajaxHooker_modifiedResponseText !== undefined) {
                    return this.__ajaxHooker_modifiedResponseText;
                }
        ...
        // 重写原型send方法
        winAh.realXHR.prototype.send = function(body) {
            const self = this;
            
            // 检查是否是fakeXHR创建的实例，如果是则跳过（fakeXHR已经处理过了）
            if (self.__ajaxHooker_fromFakeXHR) {
                return originalProtoSend.apply(this, arguments);
            }
            
            if (self.__ajaxHooker_url) {
            const request = {
                type: 'xhr',
                url: self.__ajaxHooker_url.toString(),
                method: (self.__ajaxHooker_method || 'GET').toUpperCase(),
                abort: false,
                headers: {},
                data: body,
                response: null,
            ...
            self.__ajaxHooker_request = request;
            
            // 先添加事件监听器，防止错过缓存请求的响应
            let responseHookProcessed = false;
            
            // 处理响应的函数
            const handleResponse = function() {
                if (self.readyState < 3) return;
                if (responseHookProcessed && self.readyState < 4) return;
                
                if (self.readyState === 4 && !responseHookProcessed) {
                    responseHookProcessed = true;
                    
                    // 等待hook函数完成后处理响应
                    const processResponse = function() {
                        // 检查是否已经在getter中处理过
                        if (self.__ajaxHooker_responseProcessed) return;
                        self.__ajaxHooker_responseProcessed = true;
                        
                        if (typeof request.response === 'function') {
                            const response = {
                                finalUrl: self.responseURL,
                                status: self.status,
                                responseHeaders: parseHeaders(self.getAllResponseHeaders()),
                                response: self.response,
                                responseText: self.responseText
                            };
                            try {
                                if (self.responseType === '' || self.responseType === 'document') {
                                    response.responseXML = self.responseXML;
                                }
                            } catch (e) {}
                            ...
                            if (typeof result === 'string') {
                                self.__ajaxHooker_modifiedResponseText = result;
                            } else if (result && typeof result.then === 'function') {
                                result.then(function(modifiedText) {
                                    if (typeof modifiedText === 'string') {
                                        self.__ajaxHooker_modifiedResponseText = modifiedText;
                                    }
                                }).catch(function(e) {
                    ...
                    if (request.__hookCompleted) {
                        processResponse();
                    } else {
                        request.__waitForHook = request.__waitForHook || [];
                        request.__waitForHook.push(processResponse);
                    }
                }
            };
            
            // 监听多个事件
            self.addEventListener('readystatechange', handleResponse);
            self.addEventListener('load', handleResponse);
            self.addEventListener('progress', handleResponse);
            ...
            try {
                ...
                console.error('prototype hook execution error:', e);
            }
            
            // 标记hook完成
            const completeHook = function() {
                request.__hookCompleted = true;
                if (request.__waitForHook) {
                    for (var i = 0; i < request.__waitForHook.length; i++) {
                        request.__waitForHook[i]();
                    }
                }
            };
            
            if (hookPromises.length > 0) {
                Promise.all(hookPromises).then(completeHook).catch(function(e) {
                    console.error('hook promises error:', e);
                    completeHook();
                });
            } else {
                completeHook();
    ...
    } // end if (!winAh.protoHookInstalled)
    ...
    return {
        ...
        // 检测hook是否仍然有效
        checkHook: function() {
            const isXHRHooked = win.XMLHttpRequest === winAh.fakeXHR;
            const isFetchHooked = win.fetch === winAh.fakeFetch;
            return { xhr: isXHRHooked, fetch: isFetchHooked };
        }
    };
}();